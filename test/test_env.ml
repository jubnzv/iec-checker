...
let test_add_var_decl () =
  let global_scope = Env.mk_global in
  ...
  let local_scope = Env.mk global_scope in
  Alcotest.(check int)
    "no initial variable declarations" 0
    (List.length (Env.get_vdecls local_scope));
  let local_scope = Env.add_vdecl local_scope vd1 in
  Alcotest.(check int)
    "can add variable declarations" 1
    (List.length (Env.get_vdecls local_scope));
  Alcotest.(check string)
    "added correct data" "dummy"
    ( match Env.get_vdecls local_scope with
